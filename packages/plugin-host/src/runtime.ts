...
/* Imported, never re-exported: the isolator this file builds is an internal of
 * the activation, not a name the plugin SDK has ever offered. */
import { createCallbackIsolator } from './callback-isolation'
...
export async function activatePlugin(
  ...
  // The wrapper every plugin callback is registered through (see
  // `./callback-isolation`), built here so its notice record is this run's.
  const isolate = createCallbackIsolator(id, reportPluginCallbackError)
