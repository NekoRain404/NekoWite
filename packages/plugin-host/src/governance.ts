...
export async function loadAuditLogFromFile(): Promise<boolean> {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as PluginAuditEvent[]
    ...
    // Best-effort restore: the log file may be absent (first run) or the fs may
    // return non-log content. This is a silent skip (debug, not a warning) —
    // failing to LOAD history never blocks recording NEW events going forward.
    console.debug('[NekoWite:governance] no persisted plugin audit log loaded', err)
    return false
  }
}
