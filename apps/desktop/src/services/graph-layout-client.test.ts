...
import { computeGraphLayout } from './graph-layout-client'
import { computeLayoutChunked } from './link-graph'
import type { LayoutOptions, LayoutPoint } from './link-graph'
 ...
 *  It runs the SAME layout math as {@link graph-layout-worker.ts}, but tags the
...
describe('computeGraphLayout — Web Worker path', () => {
  it('sends the job to the Worker and returns the points it posted back', async () => {
    ...
    expect(String(worker.url)).toContain('graph-layout-worker.ts')
