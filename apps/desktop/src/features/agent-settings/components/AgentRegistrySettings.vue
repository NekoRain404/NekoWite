<script lang="ts">
/**
 * The section's copy: the shape a caller may override, and the catalogue it defaults to.
 *
 * The sentences themselves are in `src/i18n/namespaces/agent.ts` (`agent.registry.*`), beside the
 * panel's command-menu and permission strings — this page has no copy of its own, so a translated
 * build is a catalogue edit rather than a second place to look. What stays here is the *shape*
 * (the keys a caller may replace through the `labels` prop) and the builder that reads the
 * catalogue into it, because a page that reached for `t()` at every use site could not be given a
 * different set of sentences at all.
 *
 * `refusal` is keyed by the backend's own refusal kinds, so an arm added to
 * `agent-registry-policy.ts` without a sentence in the catalogue is a typecheck failure rather than
 * a row that says nothing. `program` is absent on purpose: a program problem is one of the five
 * `programState` sentences, shared with the state line every row already draws — one answer per
 * state, not two that could drift.
 */
import { t } from '../../../i18n'
...
export interface AgentRegistryLabels {
  ...
  engine: { title: string; current: string; none: string; choose: string; creates: string; keeps: string; start: string }
}
...
export function registryLabels(): AgentRegistryLabels {
  return {
    ...
    engine: {
      ...
      start: template('agent.registry.engine.start', 'engine'),
    },
  }
}
</script>
