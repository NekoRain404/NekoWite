 ...
 * catalogue now live in `agent-runtime-labels.ts`, and the type is re-exported here.
 *
 * The re-export is not decoration. The specifier callers already name is *this file* —
 * `features/agent-settings/index.ts` exports `AgentRuntimeLabels` from
 * `./components/AgentRuntimeSettings.vue` — and an SFC's named type exports are what such a specifier
 ...
 */
export type { AgentRuntimeLabels } from './agent-runtime-labels'
</script>
...
import { runtimeLabels, type AgentRuntimeLabels } from './agent-runtime-labels'
