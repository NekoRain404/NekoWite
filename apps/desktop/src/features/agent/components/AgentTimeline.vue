 ...
 * The bar it draws is `./AgentTimelineSearch.vue` and the rule it applies is
 * `services/agent-conversation-search.ts`; the state between the two — what is in the box, where
 * the hits are, which one the reader is on, and what landing on one does to the container — is
 * `use-agent-find-bar`, which is where the three rulings that shape it are stated in full. It
 * moved there when this file went past the size this project allows one component, as a move:
 * the template below still places the bar, and the comments went with the code they explain.
 *
 * What stays here is the two things that are this file's own. **Where the bar sits** — a row of
 * its own above the log, which the template's own note explains. And **the one act the composable
 * cannot perform for itself**: opening a row of folded reasoning a hit has landed in, which is why
 * it takes an `unfold` rather than reaching into the transcript's disclosure. Which rows are open
 * is this component's state, and its two entry points mean different things — `toggleThought`
 * announces a height change, `unfoldThought` must not.
 */
...
import { useAgentFindBar } from '../composables/use-agent-find-bar'
...
/** Open one reasoning row, and only open it. The find bar's landing move is the one caller, and it
 *  is exactly the case where {@link toggleThought}'s announcement would be wrong: that
 *  announcement exists so a reader who is staying put keeps their place across the height change,
 *  and this reader is being taken somewhere instead — the move that follows corrects for it.
 *  Idempotent, because a hit in a row that is already open has nothing to open. */
function unfoldThought(id: number): void {
  if (!isThoughtOpen(id)) openThoughts.value = [...openThoughts.value, id]
}
...
/** The bar's own component, which the template below renders. Bound here rather than inside
 *  `use-agent-find-bar` because a template ref belongs to the markup that carries it — the
 *  composable is handed it, the way `use-agent-panel-menu` is handed its popup. */
const bar = ref<InstanceType<typeof AgentTimelineSearch> | null>(null)

/**
 * The find bar's state, and the marks the rows below are drawn with.
 *
 * Destructured rather than kept as one `find.*` binding, so the markup goes on naming each of
 * these exactly as it did while they were refs declared here. The two acts the markup reaches for
 * by hand are renamed to say what they act on; the composable's own names have no need to, since
 * it *is* the find bar. `total` is the one answer the template did not have a name for before: it
 * used to read `hits.length` off a list that now belongs to the composable.
 */
const {
  searching,
  query,
