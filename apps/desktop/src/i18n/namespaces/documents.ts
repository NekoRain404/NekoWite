...
export const documents = {
  en: {
    ...
    tabs: {
      ...
      // The close's half of the same rescue. `loadRacedTyping` would be a lie
      // here — it says the note now shows the disk — so this one says what the
      // user is left with: their text, in a tab, and a file nothing wrote to.
      // It also has to carry the reason the X did not just save: a note that
      // has not been read has no text of its own to save.
      closeWhileLoading:
        'This note was still loading, so it had no text of its own to save. What you typed is in a new untitled tab; the file was left as it is on disk.',
  ...
  zh: {
    ...
    tabs: {
      ...
      closeWhileLoading:
        '这篇笔记尚未加载完成，没有属于自己的内容可保存。你输入的正文已放进新的未命名标签；文件保持磁盘上的原样。',
