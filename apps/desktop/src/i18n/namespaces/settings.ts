    ...
    settings: {
      ...
      pet: {
        ...
        general: {
          ...
          ballNote: 'The ball sits in a corner of the screen and stays where it is put. It is a window of its own and this is the only switch that decides whether it is there: the character window’s switch does not take it down, and its state does not depend on that one.',
          ballSize: 'Ball size — {px} px',
          ballSizeNote: 'The orb is drawn at this size, and the ball’s own window is sized from it. The preview beside this page draws the character, not the ball.',
          characterWindow: 'Show the character window',
          characterWindowNote: 'The pet is drawn in this window, and this is the only switch that decides whether it is there. Switching it off leaves the floating ball on the desktop whenever the ball’s own switch is on, so a pet that is only a ball is a choice you can make.',
          window: 'Window',
        ...
        bubble: {
          ...
          themeLight: 'Light',
          themeDark: 'Dark',
          themeNote: 'A bubble on your desktop follows your system\'s light or dark preference; a bubble drawn inside the app follows the app\'s theme. Light and Dark override both.',
          fontSize: 'Text size',
          fontSizeOption: '{size}px',
          fontSizeNote: 'The bubble\'s own text size. It does not change the app\'s body size, or the pet menu\'s.',
          ...
          layout: 'How the rows are drawn',
          layoutMode: 'Rows',
          modeList: 'All rows',
          modeCarousel: 'One page at a time',
          modeCompact: 'Summary and a fold',
          maxRows: 'At most {rows} rows',
          grouping: 'Sessions',
          groupingByAgent: 'Grouped by engine',
          groupingFlat: 'One list',
          filter: 'Which states to show',
          filterAll: 'All states',
          filterAttention: 'Only what needs me',
          filterActive: 'Working and waiting',
          filterWorking: 'Working only',
          separator: 'Between the fields',
          separatorDot: 'Dot',
          separatorArrow: 'Arrow',
          separatorBar: 'Bar',
          separatorSpace: 'Space',
          fields: 'Fields on a row',
          fieldsNote: 'The order is the renderer\'s; what this sets is which of them appear.',
          preset: 'Start from a preset',
          presetOriginal: 'Original',
          presetStandard: 'Standard',
          presetDetailed: 'Detailed',
          phrases: 'What the pet says',
          phrasesHint: 'One phrase per line.',
          phrasesNote: 'These are your words, and the bubble says one of them only when there is nothing to report. An empty list is the default: the pet says nothing rather than something this build wrote for you.',
          idle: 'Say a line when nothing is running',
          tokenDot: 'State dot',
          tokenAgent: 'Engine',
          tokenSession: 'Session',
          tokenSeparator: 'Separator',
          tokenMessage: 'Phrase',
          tokenStateLabel: 'State',
          tokenElapsed: 'Elapsed',
          dot: 'State dot',
          dotPlain: 'Plain dot',
          dotClaude: 'Claude style',
          dotNote: 'Both styles use the same state colours; only the shape changes. The working state is a colour here rather than an animation, so neither style moves.',
          agentsUnavailable: 'The per-engine icons and the per-engine visibility list are not in this build: both have to be built from the agent registry that is installed right now, and this page has no registry to read.',
        },
        notification: {
    ...
    settings: {
      ...
      pet: {
        ...
        general: {
          ...
          ball: '显示悬浮球',
          ballNote: '悬浮球停在屏幕一角，放在哪里就停在哪里。它是一个独立的窗口，决定它在不在的只有这个开关：角色窗口的开关收不走它，它的状态也不取决于那一个。',
          ballSize: '悬浮球大小 — {px} px',
          ballSizeNote: '悬浮球按这个尺寸绘制，它自己的窗口大小也由此决定。右侧预览画的是角色，不是悬浮球。',
          characterWindow: '显示角色窗口',
          characterWindowNote: '桌宠就画在这个窗口里，决定它在不在的只有这个开关。关掉它，只要悬浮球自己的开关开着，悬浮球仍会留在桌面上——只开悬浮球是可以的。',
          window: '窗口',
        ...
        bubble: {
          ...
          themeLight: '浅色',
          themeDark: '深色',
          themeNote: '桌面上的气泡跟随系统的深浅色偏好，画在应用里的气泡跟随应用主题；浅色与深色可以覆盖两者。',
          fontSize: '文字大小',
          fontSizeOption: '{size}px',
          fontSizeNote: '气泡自己的文字大小。它不会改变应用的正文大小，也不会改变桌宠右键菜单的大小。',
          opacity: '背景不透明度 {pct}%',
          ...
          maxRows: '最多 {rows} 行',
          grouping: '会话',
          groupingByAgent: '按引擎分组',
          groupingFlat: '合成一列',
          filter: '显示哪些状态',
          filterAll: '全部状态',
          filterAttention: '只看需要我的',
          filterActive: '进行中与等待中',
          filterWorking: '只看进行中',
          separator: '字段之间',
          separatorDot: '圆点',
          separatorArrow: '箭头',
          ...
          separatorSpace: '空格',
          fields: '行内字段',
          fieldsNote: '顺序由渲染器决定，这里设定的是哪些字段会出现。',
          preset: '从预设开始',
          presetOriginal: '原始',
          presetStandard: '标准',
          presetDetailed: '详细',
          phrases: '桌宠说的话',
          phrasesHint: '每行一句。',
          phrasesNote: '这些是你自己写的句子，只有在没有任务可报告时气泡才会说其中一句。列表默认为空：此时桌宠什么都不说，而不是说你没写过的话。',
          idle: '没事可报告时说一句',
          tokenDot: '状态圆点',
          tokenAgent: '引擎',
          tokenSession: '会话',
          ...
          dotPlain: '实心圆点',
          dotClaude: 'Claude 样式',
          dotNote: '两种样式用同一套状态颜色，只换形状。这里「进行中」靠颜色表示而不是动画，所以两种样式都不会动。',
          agentsUnavailable: '各引擎的图标与各引擎的显示开关都不在本版本内：两者都必须按当前安装的 Agent 注册表生成，而本页面没有注册表可读。',
        },
        notification: {
          events: '提醒时机',
