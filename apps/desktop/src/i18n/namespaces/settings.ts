    ...
    settings: {
      ...
      pet: {
        ...
        bubble: {
          ...
          dotUnavailable: 'The state dot\'s two styles, sorting by engine within a group, the per-agent icons and the lines filed per engine and state are not in this build: the schema holds them, and this page draws no control for a value no surface acts on.',
        },
        notification: {
    ...
    settings: {
      ...
      pet: {
        ...
        bubble: {
          ...
          tokenStateLabel: '状态',
          tokenElapsed: '已用时',
          dotUnavailable: '状态圆点的两种样式、分组内按引擎排序、各 Agent 的图标，以及按引擎与状态分开存放的词句都不在本版本内：schema 留着它们，而本页面不为没有表面使用的值画控件。',
        },
        notification: {
          events: '提醒时机',
