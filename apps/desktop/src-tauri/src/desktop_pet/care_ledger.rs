 ...
 *
 * ## Import (the 导入回退 clause)
 *
 * [`CareLedger::import`] follows D6's shape for the write — a submission carries the revision it was
 * read at, and a stale one is a `Conflict` that writes nothing rather than a merge — and §4's rule
 * for what a repair may do: **it only raises**. A total below the one already here is kept and
 * reported as kept, a badge is added and never removed, and a file that does not parse is refused
 * whole: reading the fields that parsed and defaulting the rest is the "normalisation" that discards
 * a user's progress while looking like a success.
 */
...
pub struct CareImportReport {
    /// Fields the file raised, by [`IMPORT_FIELDS`]' names.
