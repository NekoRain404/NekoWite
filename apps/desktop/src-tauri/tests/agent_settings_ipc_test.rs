...
mod agent_runtime {
    ...
    #[allow(dead_code)]
    pub mod adapters;
    // Included because `runs` names it through `super::`: what a turn carries beside its text, and
    // what the engine's own report licenses, are decided by the same module.
    #[allow(dead_code)]
    pub mod attachments;
...
mod permission;
#[path = "agent_settings_ipc_test/provider.rs"]
mod provider;
