...
**第七批（本人）：wiki 链接不进索引（P1，功能性）**
- 真机发现（`apps/desktop/ai-lab/303-linkpanel.cjs`）：`source.md` 里写 `see [[target]] here`，打开 `target.md` 看「链接」面板的**入链**，显示「暂时没有笔记引用当前文档」。
- 根因：`extractOutlinks` 的正则只匹配 `[文本](目标)`（`LINK_RE`），wiki 语法 `[[目标]]` 没有圆括号，从未被提取。于是 wiki 链接虽然能渲染成 chip、Ctrl+点击能跳转，却对**所有依赖索引的视图**不可见：入链列表、图谱连线。（顺带确认了非缺陷项：单击 wiki 链接只放光标、Ctrl+点击才跳转，与 WYSIWYG 编辑器的惯例一致，代码注释也写明。）
- 修复：`extractOutlinks` 同时匹配 `[[目标]]` / `[[目标|别名]]`，别名作显示文本，无扩展名的目标规范为 `目标.md`（索引以 `<名字>.md` 为键，不规范化就永远匹配不上）；`#片段` 在补扩展名之前拆出、之后拼回（否则变成 `目标#片段.md`）；转义的 `\[[...]]` 保持普通文本；外部目标跳过。
- 验证：单元 5 例（别名、显式扩展名、片段、转义、外部目标，以及 `parseNoteMeta` 产出入链键），前三条在修复前失败；真机入链现在列出 `Source`。门禁：desktop 1453、editor-core 648、plugin-host 105、typecheck/lint 全绿。

**第八批（子代理审计 + 本人修复）：AI 权限管理对齐 Codex 的三件事**
- **先做了只读审计**（子代理，不改代码）：把「AI 权限」这条链路逐行核对，结论是——写入策略只管两类写入（`insert` / `replace-selection`），`ask()` 是唯一入口，但**入口之外还有路**：`aiService.accept()` 不受管、插件的 `open()` 与建议接口原样透传、`startChatCompletion` 没有任何前置检查、没有总开关、也没有 AI 操作的审计记录。审计同时点名了死代码（`search_notes` 全链路 + `contentSearch.ts` 里的一批导出）与「无消费者」的 `awaitingApproval`。
- **AI 总开关**：新增 `nekowite.ai.enabled`（只有 `'0'` 算关闭，缺失键按开启处理，因为那是旧版本留下的状态）。`decideAiWrite` 第一件事就是查它——总开关关闭时任何档位、任何已有授权都不放行；读路径（ghost writer、聊天、插件 AI 适配器）则单独检查，做到「关掉就不发请求」，正文因此不会离开本机。设置面板的 AI 分区里是真实复选框，说明文字讲清「关掉后不发请求、不写入，权限档位保留」。
- **「替换整篇文档」独立成类**：插件 `open()` 换掉整个缓冲区，破坏性高于替换选区，此前完全不经过权限。现在它是 `replace-document`，弹窗里写明是哪个插件、要做什么；由于弹窗必须异步，同步接口（`open` / `acceptSuggestion` / `setSuggestion`）改走「**必须已经是允许**」的规则：档位为「直接写入」或本次运行已授权才通过，否则抛错 + 提示改用可弹窗的插入接口（清空建议 `setSuggestion(null)` 永远允许）。这条规则把「默认档位下同步接口可以悄悄写」这个漏洞一并堵上。
- **真机跑出一个数据完整性 bug（本轮最重要的一条）**：新探针 `310-ai-gates.cjs` 用「本地端点命中次数」做证据，在 F 段复现——AI 开着时按 Tab 取回建议，然后在设置里关掉 AI，再按 Tab，**建议被插入文档**，几秒后自动保存把它写进磁盘（实测文件变成 `Hello world.PROBE`）。根因：`accept()` 没走权限。修复后同一段真机结果为「建议被丢弃、请求 0、磁盘字节不变」，`F1`/`F2` 转绿。
- **真机跑出第二个 bug：Escape 丢弃建议是死代码**。`311-ghost-escape.cjs` 用页面内监听器拿到了决定性证据：原生 Escape 到达文档级监听器时 `defaultPrevented` 已经是 `true`——编辑器自己的键位表先一步 `preventDefault`（ProseMirror 基础键位把 Escape 绑给了 `selectParentNode`），而这段代码刻意忽略「已被处理的按键」。于是「按 Esc 丢弃建议」从来不会发生，用户只能打字覆盖或接受。修复：Escape 改在**捕获阶段**处理，且仅当「焦点在编辑器内 + 确实有建议」时才接管；Tab 仍在冒泡阶段（表格里的 Tab 必须先让编辑器处理，这一点有既有用例守着）。真机复跑：ghost 从 `true` 变 `false`；`310` 的 A3 也从 FAIL 变 PASS。
- **探针自身的三个坑（都记在这里，避免下次重踩）**：① 探针结束时没有关闭 CDP 连接，进程会一直活着占住调试端口，下一个探针就会莫名卡死——现在统一 `browser.close()` + `process.exit(0)`；② 这个 WebView2 上输入事件的确认有时永远不来（Tab 会把焦点移出 web 内容），裸 `page.keyboard.press` 会把整轮测试挂死，所有按键都必须加超时竞速；③ 提示气泡 3 秒自动消失，采样太晚就会误判成「没有任何提示」（C5 第一次就是这么挂的）。
- **门禁**：desktop **1478**（新增 21 条：总开关矩阵、同步写入规则、`open()` 授权、accept 闸门、Escape 捕获），editor-core **648**、plugin-host **105**、Rust **145**，`typecheck` / `lint` / `clippy -D warnings` 全绿。打包版真机：`310` **15/15**、`311` Escape 生效、`286` 真实端点 **6/6**（思考深度「高」125 个推理增量 vs「不思考」0）、`270` **10/10**、`294` **6/6**（连跑两次；清扫时那一次 5/6 是探针冷启动的偶发，单项复跑两次均绿）、`301` **7/7**、`302` **6/6**、`303` PASS、`293` **4/4**。

**第八批补记（本人）**：把「思考深度」搬到用户真正需要它的地方——聊天面板底部（模型名旁）新增同一档位的下拉，与设置面板共用 `EFFORT_OPTIONS`（从设置面板搬进 `stores/settings.ts`，避免两份列表各自漂移）。第一次全量 e2e 立刻抓到一个真问题：搬走列表时漏了设置面板的 import，Vue 渲染告警，`console-clean` 的「走遍设置面板每个控件」用例变红；补上 import 后 137 项全绿（另外三项失败在单独复跑时全绿，是并发跑构建时的偶发）。打包版 `312-chat-depth.cjs` **5/5** 验收。

**本轮结束时仍未处理**（下一轮候选）：`search_notes` 全链路与其一次性内容搜索辅助函数目前无 UI 消费者（属死代码，建议删除或接入命令面板）；`stores/aiPermission` 的 `awaitingApproval` 计算属性没有消费者；插件治理文件以外的插件持久化「停用」开关仍不存在；AI 操作审计（谁在什么时候被允许/拒绝，落盘成可查记录）尚未实现——权限模型这轮已经闭合，审计轨迹是下一步的透明度工作。

**第八批后续（本人）：AI 操作审计、插件停用开关与死代码清理**

- **AI 操作审计（新功能）**：新增 `apps/desktop/src/services/aiAudit.ts`——容量 200 的环形缓冲（`MAX_AI_AUDIT_EVENTS`），字段 `seq` / `at` / `source` / `outcome` / `kind` / `detail` / `reason`；`detail` 走插件宿主共用的 `sanitizeAuditDetail`（`@nekowite/plugin-host`），经 `persistence` 端口持久化在 `nekowite.ai.audit`。`loadAiAuditLog` 按 `seq` 去重、接着已恢复的最大编号继续编号，存储损坏按「没有历史」处理，绝不让它变成启动失败。来源 `ghost | chat | edit | dialog | plugin`，结果 `asked | allowed | denied | blocked | granted`。接线点：`stores/aiPermission.ts`（ask / respond / grant / blocked 原因）、`services/ai.ts`（ghost writer 的两条 blocked 路径 + accept 被拦截）、`services/pluginAi.ts`（每一次插件补全都记，包括被总开关拦下的那次，新增 i18n 键 `plugin.aiDisabled`）。UI 是设置 → AI 里的「最近的 AI 活动」：最新 8 行（时间 / 来源 / 种类 / 结果 / 详情）+ 计数行（允许 / 拒绝 / 阻止）+ 清空按钮，键在 `aiperm.audit.*`。组件用字面量键表（`AUDIT_OUTCOME_KEYS` / `AUDIT_SOURCE_KEYS` / `AUDIT_KIND_KEYS`）：仓库的 i18n 齐备性测试扫描源码里的真实键字面量，动态拼出来的键没人能检查——**这条测试抓到了第一版**。为什么是 localStorage 而不是 vault 文件：拒绝大多发生在没有打开知识库的时候（全新安装、AI 关着），而记录必须跨重启存活，`persistence` 端口正是为此存在。
- **插件按知识库持久停用（新功能）**：`GovernanceFilePayload`（`services/plugins.ts`）新增 `disabled?: string[]`，受 MAC 保护地写进 `.nekowite/plugin-governance.json`；旧版本写出的文件没有这个字段，等于「什么都没关」。新 API：`isVaultPluginDisabled`（`getVaultDisabledPluginIds` 无任何调用方，已在模块拆分中作为死导出移除） / `setVaultPluginDisabled(pluginId, disabled, { vault?, reload? })`，以及 `listVaultPlugins(vault)` → `{ id, name, version, disabled, active, unstable }[]`。关闭立即生效：宿主 `deactivatePlugin` + 从 `activeVaultPluginIds` 移除 + 一条 `deactivate / disabled by the user` 审计；重新启用走一次完整的 vault 加载（撤销 → 版本策略 → 同意 → 信任 → 完整性）。加载循环在 **GATE 0 之前**跳过已关闭的插件，所以被关闭的插件不会被询问、不会被读取、也不会被执行。UI：设置新增「插件」分区（`SectionId` `plugins`，图标 `Puzzle`，键 `settings.plugins.*`），每行复选框 + 状态行（关闭 / 未运行 / 运行中 / 隔离）。
- **真机发现的 bug（本批最重要的一条）**：第一版沿用既有的 `scheduleGovernanceSave()` 持久化，而它依赖模块的 `currentVault`，后者只在 `loadGovernanceFile()` 里赋值——严格 CSP 的打包版**永远到不了那里**（CSP 门控提前返回）。真机表现：开关看着保存了、显示「已关闭」，重新加载后又变回启用。修复：按面板点名的知识库持久化（`{ vault }`），用**读-改-写**（`loadDisabledPlugins` / `saveDisabledPlugins`）保留文件里原有的记录（信任锚、撤销、摘要），并且**MAC 验不过的文件拒绝覆盖**。回归测试断言写出的 payload 能用知识库密钥验签通过、且仍含 `trustedKey` / `trustedSources` / `digests`。
- **探针工具自身的两个坑**：① `process.exit(0)` 放在 `finally` 里会把异常吞掉——失败的一轮变成静默 exit 0，退出必须放在汇总之后；② 断言「治理文件里有插件 id」时没有先确认文件存在，而探针知识库的目录名里就含这个 id，于是**文件缺失时断言也通过**。
- **死代码清理（关掉上一轮列的两条「下一轮候选」）**：后端 `search_notes` 全链路删除（Rust 命令 + 注册、`storage::file_store` 的 `search_notes` / `search_notes_with_max` / `search_notes_capped` / `walk_search` 与 `SEARCH_MAX_DIRS` / `SEARCH_MAX_DEPTH` / `SearchResults`、`FsPort.searchNotes` 契约、tauri 与 memory 两个适配器、e2e 夹具一行、只覆盖它的 Rust 测试）——它只匹配文件**路径**，应用已改为前端索引 + 全正文搜索（`services/contentSearch.ts` → `searchWithIndex`），且无人调用；Rust 测试 **70 → 68**（`fs_test`，Windows 上实际运行）。`contentSearch.ts` 去掉只被测试使用的导出（**242 → 140 行**：`ContentCandidate` / `SNIPPET_RADIUS` / `matchContent` / `buildSnippet` / `contentMatchOf` / `metaHaystack` / `contentMetaMatch` / `mapWithConcurrency` / `searchContentMatches` 移除，现役搜索用到的 `snippetAt` / `mapWithConcurrency` 转为私有），测试文件围绕真正入口重写（9 条：深正文命中、摘要省略号、并发上界、abort）。`stores/aiPermission.awaitingApproval`（零消费者）删除；`NoteListPanel.vue` 里内联的 `CONTENT_SEARCH_CONCURRENCY` 改用导入的共享常量（两份会各自漂移）。
