...
- 免安装可执行文件（最新）：`release/nekowite_0.1.0_x64.exe`（未签名），SHA-256：`4cc7dbce9126f2a35ef9856b552c701d28fbeb7bc3015ea8e4a272b1b732e112`（15.1 MB；含第十六轮的 P0 frontmatter 修复、Windows 图片路径修复、服务商请求修复与全部 P2）。**打包产物本身**同样做了真机验证（CDP 驱动 exe）：设置面板 AI 分区的两个新控件在位；在真实笔记上编辑并保存 frontmatter 后，块序列、流式序列与正文全部原样保留（这正是修复前会被静默破坏的那一块）。另记：打包版里无法通过 `import()` 加载源码模块（生产包没有模块加载器），因此「图片目录是否为相对路径」这一项在打包版无法用同一手法探测，改由 dev 构建真机验证（5/5）与单元测试覆盖；这条限制如实记录，而不是当作通过。
