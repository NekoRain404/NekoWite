...
**本轮结束时仍未处理**（下一轮候选）：`search_notes` 全链路与其一次性内容搜索辅助函数目前无 UI 消费者（属死代码，建议删除或接入命令面板）；`contentSearch` 中被 `NoteListPanel` 重复声明的并发常量（两处可能漂移）；`suppressReapply` 的一次性抑制没有 TTL；聊天面板的 `zz` 类临时测试文件已清理。
