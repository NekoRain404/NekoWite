...
- 「先忽略许可证的问题，这个最后解决」——**许可证审查暂缓，不在实施阶段作为闸门**。实施代理可以按需直接移植/改写 Zed 源码，不必先取得许可结论；许可与分发义务在交付前的独立阶段统一处理（§4 的登记义务届时补齐）。

**暂缓不等于消失。** 本文件 §2 的逐 crate 许可事实（全部 GPL-3.0-or-later）与 §4 的登记格式仍然有效，只是**不再阻塞开发**：许可结论在收尾阶段一次性做出，届时按 §4 补登每条实际移植记录、核对来源哈希、并决定分发方案。

**同时注意，技术约束与许可证无关，不会因暂缓而放宽：** §2.2 列出的 GPUI 单线程实体、`Project`、worktree、GPUI 定时器在 Vue/WebKitGTK + Tauri 中没有对应物，照搬过来无法编译。因此对 `acp.rs` 这类文件，实际动作仍然是**移植其职责划分与行为，改写宿主接口**——这不是许可要求，是编译要求。
