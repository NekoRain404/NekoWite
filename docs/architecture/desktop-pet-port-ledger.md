...
2. 上游**按沉默删除**工作/等待会话（`state.ts` 的 `active()`：`working`/`waiting` 静默超过 `STALE_ACTIVE_MS = 300_000` 即 5 分钟就从列表里**删除**，`registered` 90 秒、`done` 30 秒同理；源码注释是 "the agent died without a Stop"）→ **NekoWite 不得以沉默作为任务状态的依据。** 注意这里曾有一处记录错误已更正：上游**不是**「用沉默推断完成」——完成来自显式事件，沉默导致的是**任务从列表里消失**。更正后的结论比原来更强：**沉默既不能完成、也不能移除任务**；运行时状态为准，连接异常显示未知/中断。
...
7. **快照无法重建终态——D4/D5 不能从快照重新推导任务结果。** D1 实测：`completed` 不带 stop reason，所以「这一轮是正常结束、还是撞了 token 上限、还是被拒」在快照里**不可恢复**。重启后的桌宠必须从**宿主自己的记录**（§6.3 的提醒账本与历史）取终态；若 D4 读快照再投影，**重启前结束的任务会全部读成空**。这直接决定 §6.2 那张表在实现里落在哪一层：映射发生在**事件到达时**并落账，而不是在渲染时由快照推导。
