...
| M6 产品成熟度 | 基本完成 | 关闭前保存、恢复/快照/回收站、冲突处理、插件 SDK、恢复相关无障碍 | 编辑器完整 live-region、真正插件隔离（`appBootstrap` 已在切换 vault 前处理未命名 dirty 文档，`appLifecycle` 关窗时同样处理；`.tmp` 崩溃残留清理已实现） |
...
4. **数据恢复闭环**：启动时扫描 `.tmp`、展示可恢复版本；清理已确认无主的临时文件；vault 切换前处理未命名 dirty 文档。（已完成：`recoveryClosedLoop` + `appBootstrap`/`appLifecycle` 的脏文档确认。）
...
- `.tmp` 扫描和恢复 UI。
- ~~未命名 dirty 文档的 vault 切换流程。~~ 已实现（`appBootstrap.ts` 在提交新 vault 前、`appLifecycle.ts` 在关窗前各自确认一次）。
